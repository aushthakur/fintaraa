...
const assertLoanChatAccess = (query: any, role: string, actorId: any) => {
  ...
  if (
    ...
    !assignedAgentIds.includes(String(actorId))
  ) {
    throw new ApiError(403, "Access denied");
  }
};
...
export class LoanQueryChatController {
  ...
  static async getMessages(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      assertLoanChatAccess(query, role, actorId);
  ...
  static async sendMessage(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      assertLoanChatAccess(query, role, senderId);
